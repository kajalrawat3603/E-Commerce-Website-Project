...
app.post("/signup", validateEmail, async (req, res) => {
    try {
        ...
        if (existingUser) {
            ...
            } else if (existingUser.phone === req.body.number) {
        ...
        const token = jwt.sign({ _id: newuser._id, name: newuser.name }, jwt_secret, { expiresIn: '2h' });
        newuser.tokens = newuser.tokens.concat({ t: token });

        res.cookie("jwt", token, { httpOnly: true });
        await newuser.save();

        return res.status(201).json({ success: true, message: "Account created and logged in" });
